...
const BlogIdImage = ({ image, title }: Pick<BlogType, "image" | "title">) => {
  const [errorImage, setErrorImage] = useState<boolean>(false);

  return (
    <>
      <div className="relative w-full aspect-video max-h-[600px] rounded-xl overflow-hidden mt-5 bg-g9 flex justify-center items-center">
        {!image?.length || errorImage ? (
          ...
          <Image onError={() => setErrorImage(true)} src={`/api/image-proxy?url=${image}`} alt={title} fill sizes="100%" quality={100} className="object-cover" />
        )}
      </div>
    </>
  );
