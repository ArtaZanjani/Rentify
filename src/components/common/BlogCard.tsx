...
interface BlogCardPropsType extends BlogType {
  border?: boolean;
  adminPanel?: { status: false } | { status: true; onClick: Dispatch<SetStateAction<string | null>> };
}

const BlogCard = ({ _id, image, title, shortDesciption, createdAt, border = false, adminPanel = { status: false } }: BlogCardPropsType) => {
  ...
  return (
    <div className="relative">
      ...
      <div className={`w-full xs:w-98 h-[500px] bg-white rounded-2xl overflow-hidden flex flex-col justify-between pb-2 relative ${border && "border border-g9"}`}>
        <div className="w-full h-74.5 bg-g9 relative flex items-center justify-center">
          {!image?.length || errorImage ? (
            ...
            <Image onError={() => setErrorImage(true)} src={`/api/image-proxy?url=${image}`} alt={title} fill sizes="100%" className="object-cover" priority />
          )}
        </div>
        ...
        <div className="mt-4 px-2 flex flex-col flex-1">
          <div className="flex items-center gap-x-1">
            ...
            <p className="text-g6 text-button-s translate-y-0.5">{createdAtDate.toLocaleDateString("fa")}</p>
          </div>

          <p className="text-h6 text-g1 line-clamp-1 mt-4">{title}</p>

          <p className="text-body-xxs text-g1 line-clamp-2 mt-2">{shortDesciption}</p>
          <div className="mt-3"></div>

          <Button type="button" href={`/blog/${_id}`} width="w-full" height="h-12" variant="fill" className="mt-auto">
            ادامه مطلب
          </Button>
        </div>
      </div>
